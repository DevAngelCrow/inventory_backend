  ...
  implements ProductRepository, ProductQueriesRepository
{
    ...
    try {
      await this.prisma.client.mnt_product.create({
        data: {
          ...
          dimension_width: product.getDimensions().value()?.width ?? null,
          dimension_height: product.getDimensions().value()?.height ?? null,
          dimension_depth: product.getDimensions().value()?.depth ?? null,
          id_measurement_unit: product.getDimensions().value()?.unitId ?? null,
  ...
  private mapToDomain(p: mnt_product): Product {
    return Product.create({
      ...
      dimensions: p.dimension_width && p.dimension_height && p.id_measurement_unit ? {
        width: Number(p.dimension_width),
        height: Number(p.dimension_height),
        depth: p.dimension_depth ? Number(p.dimension_depth) : null,
        unitId: p.id_measurement_unit
      } : undefined,
  ...
  private mapToDto(
    ...
    return new ProductDto(
      ...
      p.id_category,
      p.color ?? undefined,
      p.dimension_width && p.dimension_height && p.id_measurement_unit ? {
        width: Number(p.dimension_width),
        height: Number(p.dimension_height),
        depth: p.dimension_depth ? Number(p.dimension_depth) : null,
        unitId: p.id_measurement_unit
